...
""" ******************** Copied from transformers/models/llama/modeling_llama.py of v4.36.2, for qllm ******************** """
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.config.pretraining_tp > 1:
            ...
            raise ValueError("config.pretraining_tp for LLaMA-3.1 of qllm is not implemented")
        ...
        if (not prefill_with_quant) and (q_len > 1):
            ...
            _, key_states = apply_rotary_pos_emb(query_states, key_states_fp16, cos, sin, position_ids)

            value_states = value_states_fp16.view(bsz, q_len, self.num_key_value_heads, self.head_dim).transpose(1, 2)
...
class LlamaForCausalLM_KVQuant(LlamaPreTrainedModel):
